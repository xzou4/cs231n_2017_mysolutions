...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    tmp[[range(N)], y] = -1
    b2_grad = np.sum(scores + tmp, axis = 0)
    grads['b2'] = b2_grad / N
    grads['W2'] = hidden.T.dot(scores + tmp) / N + 2 * reg * W2

    hidden_grad = (scores + tmp).dot(W2.T)
    hidden_grad[np.where(hidden == 0)] = 0
    b1_grad = np.sum(hidden_grad, axis = 0)
    grads['b1'] = b1_grad / N
    grads['W1'] = X.T.dot(hidden_grad) / N + 2 * reg * W1
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
